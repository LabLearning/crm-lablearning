  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        {/* Mobile : bouton principal pleine largeur, deux actions secondaires côte à côte */}
        <div className="grid grid-cols-2 sm:flex sm:items-center gap-2">
          <Link href="/dashboard/dossiers/nouveau"
            className="btn-primary inline-flex items-center justify-center gap-1.5 !py-2 !px-4 text-sm min-h-10 col-span-2 sm:col-span-1">
            <FolderPlus className="h-4 w-4" /> Nouveau dossier
          </Link>
          <Button variant="secondary" onClick={() => setImportOpen(true)} icon={<ClipboardPaste className="h-4 w-4" />}>
            <span className="sm:hidden">Importer</span><span className="hidden sm:inline">Importer depuis un texte</span>
          </Button>
      ...
      <div className="flex items-center gap-2 bg-white rounded-xl px-3 sm:py-2 border border-surface-200/60 max-w-md mb-5">
        ...
        <input type="text" value={search} onChange={(e) => handleSearch(e.target.value)} placeholder="Rechercher..." className="h-10 sm:h-auto bg-transparent text-sm text-surface-700 placeholder:text-surface-400 focus:outline-none flex-1 min-w-0" />
      </div>

      {/* Liste mobile : une carte par apprenant, actions dans le menu */}
      <div className="card overflow-hidden md:hidden">
        <div className="divide-y divide-surface-100">
          {filtered.map((a) => {
            const appInscriptions = getInscriptions(a.id)
            const entreprise = a.entreprise || a.client?.raison_sociale
            return (
              <div key={a.id} className="flex items-center gap-3 px-4 py-2.5">
                <Link href={`/dashboard/apprenants/${a.id}`} className="flex items-center gap-3 flex-1 min-w-0 min-h-10">
                  <Avatar firstName={a.prenom} lastName={a.nom} size="sm" />
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-medium text-surface-900 flex items-center gap-1.5 min-w-0">
                      <span className="truncate">{a.civilite} {a.prenom} {a.nom}</span>
                      {a.situation_handicap && <Accessibility className="h-3.5 w-3.5 text-brand-500 shrink-0" />}
                    </div>
                    <div className="text-xs text-surface-500 flex items-center gap-x-2 flex-wrap">
                      {entreprise && <span className="inline-flex items-center gap-1 min-w-0"><Building2 className="h-3 w-3 text-surface-400 shrink-0" /><span className="truncate">{entreprise}</span></span>}
                      {a.email && !entreprise && <span className="truncate">{a.email}</span>}
                      {appInscriptions.length > 0 && (
                        <span className="text-surface-400">{appInscriptions.length} formation{appInscriptions.length > 1 ? 's' : ''}</span>
                      )}
                    </div>
                  </div>
                </Link>
                <div className="shrink-0 -mr-2">
                  <RowMenu
                    width={208}
                    triggerClassName="h-10 w-10 flex items-center justify-center"
                    items={[
