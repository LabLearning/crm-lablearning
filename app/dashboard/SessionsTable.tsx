  ...
  return (
    <div className="card overflow-hidden">
      ...
      {sessions.length === 0 ? (
        ...
        <>
          {/* Téléphone : une carte par session, rien à faire défiler latéralement */}
                ...
                {sessions.map((s) => (
                  <Link key={s.id} href={`/dashboard/sessions/${s.id}`}
                    ...
                    <span className="text-xs text-surface-500 truncate">
                      {s.formateur ? `${s.formateur.prenom} ${s.formateur.nom}` : <span className="text-surface-500 italic">à affecter</span>}
                    </span>
