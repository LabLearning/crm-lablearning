...
import { LayoutDashboard, Building2, Banknote, LogOut, ChevronDown, UserCog, X } from '@/components/ui/icons'
...
const nav: NavItem[] = [
  ...
  { label: 'Commissions', short: 'Commissions', href: '/commissions', icon: Banknote },
...
/**
 * Enveloppe de l'espace apporteur d'affaires : en-tête, onglets, barre
 * mobile. Même gabarit que l'espace franchise, réduit à trois entrées.
 */
export function ApporteurShell({
  user, apporteurName, orgName, children, isImpersonating,
}: { user: User; apporteurName: string; orgName: string; children: React.ReactNode; isImpersonating?: boolean }) {
  const pathname = usePathname()
  const router = useRouter()
  const [menuOpen, setMenuOpen] = useState(false)
  const menuRef = useRef<HTMLDivElement>(null)
  const basePath = '/apporteur'

  const firstName = user.first_name || 'A'
  const lastName = user.last_name || ''
  const fullName = `${user.first_name || ''} ${user.last_name || ''}`.trim() || apporteurName
  ...
  async function handleStopImpersonation() {
    ...
    router.push('/dashboard/apporteurs')
    router.refresh()
  }

  const UserMenu = (
    <div ref={menuRef} className="relative">
      <button onClick={() => setMenuOpen(!menuOpen)} aria-label="Menu du compte"
        ...
        <div className="text-right hidden sm:block">
          ...
          <div className="text-[11px] text-surface-400 mt-0.5">{apporteurName}</div>
        </div>
        ...
        <div className="absolute right-0 top-full mt-1.5 w-56 bg-white rounded-xl border border-surface-200 shadow-elevated py-1 animate-in-scale origin-top-right z-50">
          <div className="px-3.5 py-2.5 border-b border-surface-100">
            ...
            <div className="text-[11px] text-surface-400 mt-1 inline-flex items-center gap-1">
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: PORTAL_GREEN }} /> {apporteurName}
            </div>
          </div>
          ...
          <div className="sticky top-0 z-40 bg-amber-500 text-white px-4 py-2.5 flex items-center justify-between">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <span className="text-sm font-medium truncate">
                Mode aperçu : vous voyez l&apos;espace de <strong>{apporteurName}</strong> ({fullName})
              </span>
            </div>
        ...
        <header className="md:hidden sticky top-0 z-30 bg-white/95 backdrop-blur-sm border-b border-surface-200/60">
          <div className="h-14 px-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <NotificationsBell userId={user.id} allHref="/apporteur/notifications" />
              {UserMenu}
            </div>
          </div>
        </header>
          ...
          <div className="max-w-screen-2xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="border-l border-surface-200 pl-3">
                <div className="text-sm font-heading font-semibold text-surface-900 leading-none">{apporteurName}</div>
                <div className="text-[10px] text-surface-400 mt-1">Espace apporteur d&apos;affaires · {orgName}</div>
              </div>
            </div>
