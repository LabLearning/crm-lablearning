 ...
 * l'accès par lien. Le `token` reste transmis aux boutons d'action (ajout /
 * changement de participant) : il identifie le formateur côté server action.
 */
export async function ApprenantsView({ formateurId, token }: { formateurId: string; token: string }) {
